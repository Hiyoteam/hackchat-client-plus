...
function join(channel, oldNick) {
  ...
  ws.onopen = function () {
    ...
    if (!wasConnected) {
      if (location.hash) {
        ...
        if (newNick !== null) {
          ...
          pushMessage( {
            nick: '!', text: "You cancelled joining. Press enter at the input field to reconnect."
          })
        }
      }
    }
    ...
    if (myNick && shouldConnect) {
      ...
      send( {
        cmd: 'join', channel: channel, nick: myNick
      });
      wasConnected = true;
  ...
  ws.onclose = function () {
    ...
    if (shouldAutoReconnect) {
      if (wasConnected) {
        wasConnected = false;
        pushMessage( {
          nick: '!', text: "Server disconnected. Attempting to reconnect. . ."
        });
      }
      ...
      window.setTimeout(function () {
        if (!wasConnected) {
          shouldAutoReconnect = false;
          pushMessage( {
            nick: '!', text: "Failed to connect to server. When you think there is chance to succeed in reconnecting, press enter at the input field to reconnect."
          })
        }
      },
        2000);
    ...
    if (args.channel) {
      if (args.channel != myChannel && isInChannel) {
        ...
        if (args.channel != 'purgatory') {
          ...
          pushMessage( {
            nick: '!', text: `Unexpected Channel ?${args.channel} . You may be kicked or moved to this channel by force. `
          })
        } else {
          purgatory = true
          pushMessage( {
            nick: '!', text: `Unexpected Channel ?${args.channel} . You may be locked out from ?${myChannel} . You may also be kicked or moved to this channel by force. `
          })
        }
      } else if (isInChannel) {
        if (purgatory && myChannel != 'purgatory') {
          // you are moved by a mod from purgatory to where you want to be at
          purgatory = false
          pushMessage( {
            nick: '!', text: `You are now at ?${args.channel} . A mod has moved you. `
          })
    ...
    if (cmd == 'join') {
      ...
      if (localStorageGet('joined-left') != 'false') {
        if (count > 5 && $id('joined-left').checked) {
          ...
          pushMessage( {
            nick: '*', text: 'Frequent joining detected. Now temporarily disabling join/left notice.'
          })
        } else if (count < 5 && !($id('joined-left').checked)) {
          $id('joined-left').checked = true
          pushMessage( {
            nick: '*', text: 'Auto enabled join/left notice.'
          })
        }
      }
    }
...
var COMMANDS = {
  ...
  onlineSet: function (args, raw) {
    ...
    pushMessage( {
      nick: '*',
      text: i18ntranslate("Users online: ", 'system') + nicksHTML.join(", ")
    },
      {
        i18n: false,
        isHtml: true,
    ...
    pushMessage( {
      nick: '*',
      text: "Thanks for using hackchat++ client! Source code at: https://github.com/Hiyoteam/hackchat-client-plus"
    },
      {
        i18n: true
      })
...
function makeTripEl(args, options, date) {
  ...
  if (args.flair) {
    // https://github.com/hack-chat/main/commit/dc841cb25e2efd456811f8e01cdc60d825c0f2b4
...
function getUpdateMessageLastText(customId, userid) {
  return activedMessages.reverse().find((msg) => {
    return msg.customId == customId && msg.userid == userid
  })?.text;
}
...
function pushMessage(args, options = {}, padId = "messages", makeunread = true, in_log = true) {
  ...
  if (
    typeof (myNick) === 'string' && (
      (args.text.match(new RegExp('@' + currentNick + '\\b', "gi")) && args.nick != myNick) ||
