...
function join(channel) {
	...
	ws.onopen = function () {
		...
		wasConnected = true;
		pushMessage({ nick: '*', text: "Thank you for using hackchat\\+\\+!"})
	}
...
function pushMessage(args) {
	...
	if (verifyNickname(myNick) && args.nick == myNick.split('#')[0]) {
...
function userInvite(nick) {
	target = prompt('target channel:') || null
	if (target){
		send({ cmd: 'invite', nick: nick, to: target });
	} else {send({ cmd: 'invite', nick: nick });}
}
